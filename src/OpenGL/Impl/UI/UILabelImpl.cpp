...
void UILabelImpl::render() {
    ...
    FontRenderer::getTextBoundingBox("WenQuanYiMicroHei", 20, mText, &fontW,
                                     &fontH);
    float offsetX = (mWidth - fontW) / 2.f;
    float offsetY = (mHeight - fontH) / 2.f;
    FontRenderer::addText("WenQuanYiMicroHei", 20, mPosX + offsetX,
                          mPosY + fontH + offsetY, Vector3(0.1f, 0.1f, 0.1f),
                          mText);
...
void UILabelImpl::prepareRenderData() {
    ...
    textureId =
        textureManager->getTexture("media/textures/FFFFFF-1.png").glTextureID;
}