...
UIImpl::UIImpl() {
    ...
    UIHintLabel =
        addLabel(UI_HINT_LABEL_ID, 0, 0, 0, 0, "UIHint", Vector4(1, 1, 1, 0.5));
}
UIButton* UIImpl::addButton(int id, int x, int y, int width, int height,
                            uint32_t textureID_idle, uint32_t textureID_hover,
                            uint32_t textureID_press, const char* text,
                            void (*callback)(void* data), void* userData,
                            UINode* parent) {
    UIButtonImpl* button;
    if (parent == NULL)
        button = new UIButtonImpl(mRootNode);
    else
        button = new UIButtonImpl(parent);
    button->nodeID = id;
...
UILabel* UIImpl::addLabel(int id, int x, int y, int width, int height,
                          const char* text, Vector4 color, UINode* parent) {
    UILabelImpl* label;
    if (parent == NULL)
        label = new UILabelImpl(mRootNode);
    else
        label = new UILabelImpl(parent);
...
void UIImpl::render() {
    ...
    if (strcmp(uiHintText, "") != 0) {
        ...
        FontRenderer::getTextBoundingBox("WenQuanYiMicroHei", 20, uiHintText,
                                         &fontW, &fontH);
